function pcFromModelnet(outfileroot,vertex,category,offset,threshold,ybias,zbias,threshold_shift)
...
%     end
    ...
    %translate to origin
    vertex_new(:,1) = vertex(:,1) - min(vertex(:,1))+1;
    vertex_new(:,2) = vertex(:,2) - min(vertex(:,2))+1;
    vertex_new(:,3) = vertex(:,3) - min(vertex(:,3))+1;
    
    ind = find(vertex(:,1) < 40);
    ...
    for i = 1:size(x,1)
        if x(i) > 0 && x(i) < 50 && y(i) > 0 && y(i) < 50 && z(i) > 0 && z(i) < 50
