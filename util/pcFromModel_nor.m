function pcFromModel_nor(outfileroot,OBJ,category,offset,threshold,ybias,zbias,lookat,len)
...
for index = 1:size(parameters,1)
    ...
    [xc,yc,~,~] = circlefit(vertex_new(:,2)',vertex_new(:,3)');
    sy = round(xc) - len/2;
    sz = round(yc) - len/2;
    ...
    answer = zeros(len,len,len);
    for i = 1:size(x,1)
        if x(i) > 0 && x(i) < len && y(i) > 0 && y(i) < len && z(i) > 0 && z(i) < len
            check = rand();     %check if this pixel will shift
            shift = 0;
            if check > threshold
                continue;             
%                 shift = round(rand()*10-5); %shift:-5~5
            else
                z(i) = z(i)+shift;
                z(i) = max(1,z(i));
...
function vertex_new = normalize(vertex,len)
x = vertex(:,1);
y = vertex(:,2);
z = vertex(:,3);
scale = len/(max(x)-min(x));
xt = (min(x) + max(x))/2;
yt = (min(y) + max(y))/2;
zt = (min(z) + max(z))/2;
x = (x - xt)*scale + xt;
