...
for iline=1:length(ftype)
    ...
    iline;
    %     for i = 1:size(data,2)
    %         if ~isa(data,'double') && findstr(data,'#QNAN')
    %             data(i) = str2num(strrep(data(i),'#QNAN',''));
    %         end
    %     end
    
    %for MODELNET only
%     if ~isa(data,'double')&& sum(cellfun(@(s) ~isempty(findstr('#QNAN',s)),data)) ~= 0%~isempty(findstr(cell2mat(data),'#QNAN'))
%         index = cellfun(@(s) ~isempty(findstr('#QNAN',s)),data);
%         data(index) = strrep(data(index),'#QNAN','');
%         data(index) = num2cell(str2double(data(index)));
%         data = cell2mat(data(:));
%     end
